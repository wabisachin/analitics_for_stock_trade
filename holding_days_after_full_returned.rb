...
def holding_days_after_full_returned(stock_code)
    ...
    total_profit_rate_buy = 0 #買いエントリー合計損益(割合ベース)
    total_profit_sell = 0 #売りエントリー合計損益
    total_profit_rate_sell = 0 #売りエントリー合計損益(割合ベース)
    total_profit_win_buy =  0 #買いエントリーの勝ち額の総和
    total_profit_rate_win_buy =  0 #買いエントリーの勝ち額の総和(割合ベース)
    total_profit_lose_buy =  0 #買いエントリーの負け額の総和
    total_profit_rate_lose_buy =  0 #買いエントリーの負け額の総和(割合ベース)
    total_profit_win_sell =  0 #売りエントリーの勝ち額の総和
    total_profit_rate_win_sell =  0 #売りエントリーの勝ち額の総和(割合ベース)
    total_profit_lose_sell =  0 #売りエントリーの負け額の総和
    total_profit_rate_lose_sell =  0 #売りエントリーの負け額の総和(割合ベース)
    ...
    rate_in_entity_for_sticklength =  0.8
    ...
    (count-(days_in_holding-1)-1).times do |i|
        ...
        if white_stick?(data_yesterday)
            ...
            # entry_price = data_today[:open]
            # exit_price = data_for_position_closed_day[:close]
            
            # #ストップロスの設定とストップロスタッチの判定
            # stop_loss_price = data_yesterday[:high]
            # days_in_holding.times do |j|
            #     day = table[i+j+1]
            #     if stop_loss_price < day[:open]
            #         # puts stock_code
            #         # puts data_today[:date]
            #         # puts "寄り付きストップロス!"
            #         loss_cut_price = day[:open]
            #         exit_price = loss_cut_price
            #         break
            #     elsif stop_loss_price < day[:high]
            #         # puts stock_code
            #         # puts data_today[:date]
            #         # puts "場中ストップロス!"
            #         loss_cut_price = stop_loss_price
            #         exit_price =  loss_cut_price
            #         break
            #     end
            # end
            ...
            #     total_profit_rate_win_sell += trade_profit_rate
            # else 
            #     total_profit_lose_sell += trade_profit
            #     total_profit_rate_lose_sell += trade_profit_rate
            # end
            ...
            # total_profit_rate_sell += trade_profit_rate
            # count_sell += 1
            # count_all +=1
            ...
            # # puts total_profit
            # trade = {date: Date.parse(data_today[:date]),type: "sell", entry_price: entry_price, exit_price: exit_price}
            # trades.push trade
        else
            ...
            exit_price = data_for_position_closed_day[:close]
            #ストップロスの設定とストップロスタッチの判定
            stop_loss_price = data_yesterday[:low]
            days_in_holding.times do |j|
                day = table[i+j+1]
                if stop_loss_price > day[:open]
                    # puts stock_code
            ...
            if trade_profit > 0
                ...
                total_profit_rate_win_buy += trade_profit_rate
            else 
                total_profit_lose_buy += trade_profit
                total_profit_rate_lose_buy += trade_profit_rate
            end
            ...
            total_profit_rate_buy += trade_profit_rate
            count_buy += 1
            count_all +=1
            total_profit += trade_profit
            total_profit_rate += trade_profit_rate
            ...
            # print ":"
            # puts data_for_position_closed_day[:close]
            trade = {date: Date.parse(data_today[:date]),type: "buy", entry_price: entry_price, exit_price: exit_price}
            trades.push trade
            # puts total_profit
    ...
    result = {
        ...
        profit_rate_buy: total_profit_rate_buy,
        profit_sell: total_profit_sell,
        profit_rate_sell: total_profit_rate_sell,
        profit_win_buy: total_profit_win_buy,
        profit_rate_win_buy: total_profit_rate_win_buy,
        profit_lose_buy: total_profit_lose_buy,
        profit_rate_lose_buy: total_profit_rate_lose_buy,
        profit_win_sell: total_profit_win_sell,
        profit_rate_win_sell: total_profit_rate_win_sell,
        profit_lose_sell: total_profit_lose_sell,
        profit_rate_lose_sell: total_profit_rate_lose_sell,
