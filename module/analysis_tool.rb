...
def gap_up?(data_ohlc_today, data_ohlc_yesterday,distance=0)
    if data_ohlc_today[:open] >= data_ohlc_yesterday[:close] + distance
        return true
    else 
        return false
    end
...
def gap_up_for_rate?(data_ohlc_today, data_ohlc_yesterday,rate=0)
    if (data_ohlc_today[:open]-data_ohlc_yesterday[:close])/data_ohlc_yesterday[:close].to_f > rate
        return true
    else
        return false
    end
...
def gap_down_for_rate?(data_ohlc_today, data_ohlc_yesterday,rate=0)
    if (data_ohlc_yesterday[:close] - data_ohlc_today[:open])/data_ohlc_yesterday[:close].to_f > rate
        return true
    else
        return false
    end
